 ...
 * - `chunk`: Potongan teks dari respons AI.
 * - `tool_call`: Permintaan dari AI untuk menjalankan tool.
 * - `tool_result`: Hasil dari eksekusi tool yang dikirim kembali ke AI.
 * - `cached_response`: Seluruh respons diambil dari cache.
 * - `error`: Menandakan terjadi error.
 */
...
export class Cerebrum {
  ...
  constructor(
    ...
    // Gabungkan provider bawaan dengan provider kustom dari pengguna
    const allProviderConfigs = { ...builtInProviderConfigs, ...config.customProviders };
    ...
    this.allProviderConfigs = allProviderConfigs;

    // Validasi bahwa semua provider dalam strategi memiliki definisi
    ...
    // @ts-ignore - 'adapters' sengaja tidak ada di skema Zod utama untuk fleksibilitas
  ...
  public async bootstrap(): Promise<void> {
    ...
    healthMonitor.start(apiManager, this.config);
  ...
  public async *chatStream(sessionId: string, userInput: string, options: ChatOptions = {}): AsyncGenerator<ChatStreamEvent, void, unknown> {
    ...
    try {
      ...
      while (toolCallCount++ < MAX_TOOL_CALLS) {
        ...
        const coreIdentityPrompt = `${this.corePrompt}\n\n[PERINTAH MUTLAK UNTUK AI]: Anda HARUS selalu bertindak dan mengidentifikasi diri sebagai 'Cerebrum'. Jangan pernah menyebut nama model (seperti GPT, Llama) atau provider (seperti OpenAI, Groq) di dalam respons Anda kepada pengguna, kecuali jika ditanya secara spesifik. Peran Anda adalah sebagai framework orkestrasi cerdas.`;
        const finalSystemPrompt = `${coreIdentityPrompt}\n\n[Instruksi Tambahan dari Pengguna untuk Tugas Ini]: ${userSystemPrompt}`.trim();

        const prunedHistory = this._pruneHistory(currentHistory);
        const initialHistory: Message[] = [{ role: 'system', content: finalSystemPrompt }, ...prunedHistory];
        
        await this._executePlugins('onPreRequest', { ...baseContext, history: initialHistory });
        
        const { message, provider } = await responder.generateNextMessage(
          initialHistory, 
          this.config, 
          this.allProviderConfigs, 
          this.config.tools,
          options.toolChoice
        );
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
          ...
          for await (const chunk of responder.streamFinalResponse(message)) {
             fullResponse += chunk;
